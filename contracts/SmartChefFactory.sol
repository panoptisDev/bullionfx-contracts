...
// File: IBullionFXProfile.sol

/**
 * @title IBullionFXProfile
 */
interface IBullionFXProfile {
...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when BULL mining ends.
    ...
    // BullionFX profile
    IBullionFXProfile public bullionFXProfile;

    // BullionFX Profile is requested
    bool public bullionFXProfileIsRequested;

    // BullionFX Profile points threshold
    uint256 public bullionFXProfileThresholdPoints;
    ...
    /**
     * @notice Constructor
     * @param _bullionFXProfile: BullionFX Profile address
     * @param _bullionFXProfileIsRequested: BullionFX Profile is requested
     * @param _bullionFXProfileThresholdPoints: BullionFX Profile need threshold points
     */
    function initialize(
        address _bullionFXProfile,
        bool _bullionFXProfileIsRequested,
        uint256 _bullionFXProfileThresholdPoints
        ...
        IBullionFXProfile(_bullionFXProfile).getTeamProfile(1);
        bullionFXProfile = IBullionFXProfile(_bullionFXProfile);

        // if bullionFXProfile is requested
        bullionFXProfileIsRequested = _bullionFXProfileIsRequested;

        // bullionFXProfile threshold points when profile & points are requested
        bullionFXProfileThresholdPoints = _bullionFXProfileThresholdPoints;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(
            (!bullionFXProfileIsRequested &&
                bullionFXProfileThresholdPoints == 0) ||
                bullionFXProfile.getUserStatus(msg.sender),
            "Deposit: Must have an active profile"
        );

        uint256 numberUserPoints = 0;

        if (bullionFXProfileThresholdPoints > 0) {
            (, numberUserPoints, , , , ) = bullionFXProfile.getUserProfile(
                msg.sender
            );
        }
        ...
        require(
            bullionFXProfileThresholdPoints == 0 ||
                numberUserPoints >= bullionFXProfileThresholdPoints,
    ...
    function updateProfileAndThresholdPointsRequirement(
        ...
        bullionFXProfileIsRequested = _isRequested;
        bullionFXProfileThresholdPoints = _thresholdPoints;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 bullReward = multiplier * rewardPerBlock;
            uint256 adjustedTokenPerShare = accTokenPerShare +
                (bullReward * PRECISION_FACTOR) /
                stakedTokenSupply;
            return
...
contract SmartChefFactory is Ownable {
    ...
    function deployPool(
        ...
        bytecode = abi.encodePacked(
            ...
            abi.encode(
                _bullionFXProfile,
                _bullionFXProfileIsRequested,
                _bullionFXProfileThresholdPoints
            )
        );

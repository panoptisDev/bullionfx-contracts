...
contract SmartChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accBullPerShare; // Accumulated BULLs per share, times 1e26. See below.
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accBullPerShare = accBullPerShare.add(
                bullReward.mul(1e26).div(lpSupply)
            );
        }
        return user.amount.mul(accBullPerShare).div(1e26).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accBullPerShare).div(1e26);
    ...
    function withdraw(uint256 _amount) public {
        ...
        updatePool(0);
        uint256 pending = user.amount.mul(pool.accBullPerShare).div(1e26).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            rewardToken.safeTransfer(address(msg.sender), pending);
